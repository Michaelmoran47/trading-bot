...
class Backtester:
    ...
    def prepare_features(self, df):
        ...
        # Drop non-feature columns (including raw OHLCV data)
        columns_to_drop = ['timestamp', 'target', 'open', 'high', 'low', 'close', 'volume']
        feature_df = df.drop([col for col in columns_to_drop if col in df.columns], axis=1, errors='ignore')
    ...
    def execute_trade(self, action, price, timestamp):
        ...
        if action == 'buy' and self.cash > 0:
            ...
            amount_bought = (self.cash - fee) / price
            self.asset_holdings += amount_bought
            self.cash = 0
            self.trades.append({
                'timestamp': timestamp,
                'action': 'BUY',
                ...
                'fee': fee,
                'portfolio_value': self.asset_holdings * price
            })
        elif action == 'sell' and self.asset_holdings > 0:
            cash_received = self.asset_holdings * price
    ...
    def run_backtest(self, df, test_split=0.8):
        ...
        for i in range(len(predictions)):
            ...
            if self.asset_holdings > 0:
                portfolio_value = self.asset_holdings * current_price
            else:
                portfolio_value = self.cash
    ...
    def plot_results(self, portfolio_df):
        ...
        # Plot 1: Portfolio value vs asset price
        ax1_twin = ax1.twinx()
        ...
        ax1_twin.plot(portfolio_df['timestamp'], portfolio_df['price'],
                      label=f'{ASSET} Price', color='orange', alpha=0.6)
        ax1.set_ylabel('Portfolio Value ($)', fontsize=12)
        ax1_twin.set_ylabel(f'{ASSET} Price ($)', fontsize=12)
        ax1.set_title(f'Backtest Results: Portfolio Value vs {ASSET} Price', fontsize=14, fontweight='bold')
        ...
        print(f"{Colors.GREEN}✓ Plot saved to results/backtest_results.png{Colors.END}")
