...
def load_and_prepare_data(filename=None):
    """
    ...
    # Drop target AND raw OHLCV columns (keep only engineered features)
    columns_to_drop = ['target', 'open', 'high', 'low', 'close', 'volume']
    ...
    print(f"Baseline accuracy (always predict majority class): {y.value_counts().max() / len(y):.4f}")
...
if __name__ == "__main__":
    # Load data
    X, y = load_and_prepare_data(FEATURES_PATH)
